use crate::anime_home::{
    ...
    ComicInSearch, ComicRankListItem, ComicType, ComicUpdateListItem, Comment, LoginData,
    NewsCategory, NewsListItem, NovelCategory, NovelDetail, NovelInFilter, NovelInSearch,
    NovelVolume, ObjType, Sort, Subscribed, TaskIndex,
};
use crate::context::{get_image_cache_dir, CLIENT};
...
fn map_anyhow(err: anyhow::Error) -> std::io::Error {
    std::io::Error::new(std::io::ErrorKind::Other, err.to_string())
}

#[napi]
pub async fn init(data_path: String) {
    context::init_root(data_path.as_str()).await;
}
...
pub async fn load_cache_image(
    ...
    if let Some(model) = image_cache::load_image_by_url(url.clone())
        .await
        .map_err(map_anyhow)?
    {
        image_cache::update_cache_time(url)
            .await
            .map_err(map_anyhow)?;
        Ok(LocalImage {
        ...
        let model = image_cache::insert(model).await.map_err(map_anyhow)?;
        Ok(LocalImage {
            abs_path,
            local_path: model.local_path,
            image_format: model.image_format,
...
pub async fn load_comic_view_logs(page: i64) -> Result<Vec<ComicViewLog>> {
    let db_logs = comic_view_log::load_view_logs(page)
        .await
        .map_err(map_anyhow)?;
    Ok(db_logs
        .iter()
...
pub async fn view_log_by_comic_id(comic_id: i32) -> Result<Option<ComicViewLog>> {
    Ok(
        match comic_view_log::view_log_by_comic_id(comic_id)
            .await
            .map_err(map_anyhow)?
        {
            None => None,
...
pub async fn load_novel_view_logs(page: i64) -> Result<Vec<NovelViewLog>> {
    let db_logs = novel_view_log::load_view_logs(page)
        .await
        .map_err(map_anyhow)?;
    Ok(db_logs
        .iter()
...
pub async fn view_log_by_novel_id(novel_id: i32) -> Result<Option<NovelViewLog>> {
    Ok(
        match novel_view_log::view_log_by_novel_id(novel_id)
            .await
            .map_err(map_anyhow)?
        {
            None => None,
...
pub async fn auto_clean(time: i64) -> Result<()> {
    ...
    loop {
        let caches: Vec<image_cache::Model> = image_cache::take_100_cache(time.clone())
            .await
            .map_err(map_anyhow)?;
        if caches.is_empty() {
            break;
        ...
        for cache in caches {
            ...
            image_cache::delete_by_url(cache.url)
                .await
                .map_err(map_anyhow)?; // 不管有几条被作用
            let _ = std::fs::remove_file(local); // 不管成功与否
        }
    }
    web_cache::clean_web_cache_by_time(time)
        .await
        .map_err(map_anyhow)?;
    crate::database::cache::vacuum().await.map_err(map_anyhow)?;
    Ok(())
}
...
pub async fn create_download(buff: ComicDetail) -> Result<String> {
    download_thread::create_download(buff)
        .await
        .map_err(map_anyhow)
}
...
pub async fn download_comic_chapters_by_comic_id(id: i32) -> Result<Vec<ComicChapter>> {
    download_thread::download_comic_chapters_by_comic_id(id)
        .await
        .map_err(map_anyhow)
}
...
pub async fn delete_download(id: i32) -> Result<String> {
    download_thread::delete_download(id)
        .await
        .map_err(map_anyhow)
}
...
pub async fn renew_all_downloads() -> Result<String> {
    download_thread::renew_all_downloads()
        .await
        .map_err(map_anyhow)
}
...
pub async fn task_index() -> Result<TaskIndex> {
    CLIENT.read().await.task_index().await.map_err(map_anyhow)
}

#[napi]
pub async fn task_sign() -> Result<()> {
    CLIENT.read().await.task_sign().await.map_err(map_anyhow)
}
